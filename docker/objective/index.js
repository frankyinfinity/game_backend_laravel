...
console.log(`Objective service started.`);
...
function callCheckObjective() {
  if (!sessionCookie) {
    console.log('No session cookie, skipping objective/check...');
    scheduleNextCycle();
    return;
  }

  const path = `/api/auth/game/objective/check`;
