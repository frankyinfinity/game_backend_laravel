    ...
    public function handle(): void
    {
        ...
        try {
            ...
            if ($containers->isEmpty()) {
                \Log::warning("Nessun container trovato per il player {$this->player->id}");
                return;
            }

            // Termina ogni container
            foreach ($containers as $containerRecord) {
                try {
                    $docker->containerStop($containerRecord->container_id);
                    \Log::info("Container {$containerRecord->container_id} terminato per entity {$containerRecord->parent_id}");
                } catch (\Exception $e) {
