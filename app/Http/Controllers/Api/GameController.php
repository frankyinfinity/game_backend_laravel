...
class GameController extends Controller
{
    ...
    public function checkObjective(Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        if ($inProgressTargets->isEmpty()) {
            return response()->json([
                'success' => true,
                'message' => 'Nessun obiettivo in_progress da verificare',
                'completed_target_player_ids' => [],
                'unlocked_target_player_ids' => [],
                'completed_phase_player_ids' => [],
                'unlocked_phase_player_ids' => [],
                'completed_age_player_ids' => [],
                'unlocked_age_player_ids' => [],
                'updated_scores' => [],
            ]);
        }
        ...
        \Illuminate\Support\Facades\DB::transaction(function () use (
            ...
            foreach ($phaseByAge as $ageId => $phaseGroup) {
        ...
        return response()->json([
            'success' => true,
            'completed_target_player_ids' => array_values(array_unique($completedTargetIds)),
            'unlocked_target_player_ids' => array_values(array_unique($unlockedTargetIds)),
            'completed_phase_player_ids' => array_values(array_unique($completedPhaseIds)),
            'unlocked_phase_player_ids' => array_values(array_unique($unlockedPhaseIds)),
            'completed_age_player_ids' => array_values(array_unique($completedAgeIds)),
            'unlocked_age_player_ids' => array_values(array_unique($unlockedAgeIds)),
