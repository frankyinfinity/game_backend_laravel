...
@section('title', 'Componente Element')
...
<div class="card card-outline card-primary shadow-sm">
    <div class="card-header pb-0 d-flex align-items-center justify-content-between">
        <h4 class="mb-0 text-dark font-weight-bold">Componente Element</h4>
    </div>
    <div class="card-body">
        <div class="row">
            <div class="col-md-12 mb-4">
                <div class="row justify-content-end">
                    <div class="col-md-3 col-sm-6 mb-2">
                        <button type="button" class="btn btn-danger btn-block btn-sm js-delete shadow-sm" data-list="table_list" data-url="{{ route('element-components.delete') }}">
                            <i class="fa fa-trash"></i> Elimina Selezionati
                        </button>
                    </div>
                    <div class="col-md-3 col-sm-6 mb-2">
                        <a href="{{ route('element-components.create') }}">
                            <button type="button" class="btn btn-primary btn-block btn-sm shadow-sm"><i class="fa fa-plus"></i> Nuovo Componente</button>
                        </a>
                    </div>
                </div>
            </div>
...
<script>
    $(document).ready(function () {
        ...
        $(document).on('click', '.btn_edit', function (e) {
            var url = "{{ route('element-components.edit', ['_id_']) }}";
        ...
        var table = $("#table_list").DataTable({
            ...
            ajax: {
                type: 'POST',
                url: '{{ route('element-components.datatable') }}',
            ...
            columnDefs: [
                {
                    render: function (data, type, row) {
                        ...
                        return '<div class="form-check"><input class="form-check-input" type="checkbox" id="sel-' + row.id + '" name="selected[]" value="' + row.id + '" ' + disabledAttr + '></div>';
                    },
                    targets: 0
                },
                {
                    render: function (data, type, row) {
                        ...
                        if (row.state == 0) {
                            if (!row.image) {
                                ...
                                toggleBtn = '<form action="{{ route('element-components.toggle-state', ['_id_']) }}" method="POST" style="display:inline-block;" class="state-form js-confirm-complete mr-1">@csrf<input type="hidden" name="id" value="' + data + '"><input type="hidden" name="state" value="1"><button type="submit" class="btn btn-success btn-sm" data-toggle="tooltip" title="Termina Disegno e Blocca"><i class="fas fa-check-circle"></i></button></form>';
                            }
                        } else if (row.state == 1) {
                            toggleBtn = '<form action="{{ route('element-components.toggle-state', ['_id_']) }}" method="POST" style="display:inline-block;" class="state-form js-confirm-complete-ancore mr-1">@csrf<input type="hidden" name="id" value="' + data + '"><input type="hidden" name="state" value="2"><button type="submit" class="btn btn-primary btn-sm" data-toggle="tooltip" title="Termina Ancore e Blocca"><i class="fas fa-check-double"></i></button></form>';
                        }
        ...
        $(document).on('submit', '.js-confirm-complete', function (e) {
            if (!confirm("Sei sicuro di voler terminare il disegno e bloccare questo componente? Questa azione è irreversibile.")) e.preventDefault();
        });

        $(document).on('submit', '.js-confirm-complete-ancore', function (e) {
            if (!confirm("Sei sicuro? Questo bloccherà definitivamente le ancore.")) e.preventDefault();
        });
    });
</script>
@stop
