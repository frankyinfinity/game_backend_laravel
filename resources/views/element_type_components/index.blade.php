            ...
            <div class="col-md-12 mb-4">
                <div class="row justify-content-end">
                    <div class="col-md-3 col-sm-6 mb-2">
                        <button type="button" class="btn btn-danger btn-block btn-sm js-delete shadow-sm" data-list="table_list" data-url="{{ route('element-type-components.delete') }}">
                            <i class="fa fa-trash"></i> Elimina Selezionati
                        </button>
                    </div>
                    <div class="col-md-3 col-sm-6 mb-2">
                        <a href="{{ route('element-type-components.create') }}">
...
<script>
    $(document).ready(function () {
        ...
        $(document).on('click', '.btn_edit', function (e) {
            var url = "{{ route('element-type-components.edit', ['element_type_component' => '_id_']) }}";
        ...
        var table = $("#table_list").DataTable({
            ...
            ajax: {
                type: 'POST',
                url: '{{ route('element-type-components.datatable') }}',
                headers: {
            ...
            columnDefs: [
                {
                    render: function (data, type, row) {
                        return '<div class="form-check"><input class="form-check-input" type="checkbox" id="sel-' + row.id + '" name="selected[]" value="' + row.id + '"></div>';
                    },
                    targets: 0
                },
                {
                    render: function (data, type, row) {
                        ...
                        var deleteForm = '<form action="{{ route('element-type-components.destroy', ['element_type_component' => '_id_']) }}" method="POST" style="display:inline-block;" class="delete-form mr-1">' +
                            '@csrf' +
                            '@method("DELETE")' +
