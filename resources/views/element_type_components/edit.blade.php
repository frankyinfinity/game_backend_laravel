...
<div class="card card-outline card-primary shadow-sm">
    <div class="card-header pb-0">
        <h4 class="mb-0 text-dark font-weight-bold">Modifica Tipologia Componente</h4>
    </div>
    <div class="card-body">
        <form method="POST" action="{{ route('element-type-components.update', $elementTypeComponent) }}">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-md-6 col-12 mb-3">
                    ...
                    <div class="input-group mb-3">
                        ...
                        <input type="text" id="name" name="name" class="form-control" placeholder="Es. Struttura, Funzione..." value="{{ old('name', $elementTypeComponent->name) }}" required>
                    </div>
                </div>
                    ...
                    <div class="input-group mb-3">
                        <select id="symbol" name="symbol" class="form-control select2" required style="width: 100%;">
                            ...
                            @foreach($icons as $class => $label)
                                <option value="{{ $class }}" {{ old('symbol', $elementTypeComponent->symbol) == $class ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
