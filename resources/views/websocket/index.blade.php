...
@section('title', 'WebSocket Players')
    ...
    <script>
        $(document).ready(function () {
            var table = $("#table_list").DataTable({
                ...
                ajax: {
                    type: 'POST',
                    url: '{{ route('websocket.players.datatable') }}',
                    headers: {
                ...
                columnDefs: [
                    ...
                    {
                        render: function(data) {
                            var url = "{{ route('websocket.show', ['_id_']) }}";
                            url = url.replace('_id_', data);
                            return '<a href="' + url + '">' +
                                '<button type="button" class="btn btn-primary btn-block btn-sm"><i class="fa fa-bolt"></i> Dashboard</button>' +
                                '</a>';
                        },
                        targets: 4
                    },
