...
@section('content')
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Nuovo Elemento</h3>
        </div>
            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    <div class="custom-control custom-checkbox">
                        <input type="checkbox" class="custom-control-input" id="consumable" name="consumable" {{ old('consumable') ? 'checked' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control @error('element_type_id') is-invalid @enderror" 
                        ...
                        @foreach($elementTypes as $type)
                            <option value="{{ $type->id }}" {{ old('element_type_id') == $type->id ? 'selected' : '' }}>
                                {{ $type->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control @error('climates') is-invalid @enderror" 
                        ...
                        @foreach($climates as $climate)
                            <option value="{{ $climate->id }}" {{ (collect(old('climates'))->contains($climate->id)) ? 'selected' : '' }}>
                                {{ $climate->name }}
                            </option>
                        @endforeach
                    </select>
